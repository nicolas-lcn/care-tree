...
exports.getChallenges = (page) => {
  ...
  var results = db.prepare('SELECT challenge.id, title, description, nbUpvotes, category.name AS categoryName, state, username '
                           ...
                           + 'ORDER BY challenge.id LIMIT ? OFFSET ?').all(num_per_page, (page - 1) * num_per_page);
